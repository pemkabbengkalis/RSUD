...
<div class="dd" id="nestable3" style="max-height:55vh;overflow:auto">
      <ol class="dd-list main-list">
        @php $menu = collect(json_decode($looping_data,true));@endphp
           ...
           @foreach(json_decode(json_encode($menu->where('parent',0))) as $y=> $l)


          <li class="dd-item dd3-item menu-id-{{$l->id}}" data-id="{{$l->id}}">
            <input  type="hidden" name="id[]" value="{{$l->id ?? null}}">
            <input  type="hidden" name="parent[]" value="{{$l->parent?? null}}">
            <input  type="hidden" class="name-{{$l->id}}"  name="name[]" value="{{$l->name ?? null}}">
            <input  type="hidden" class="desc-{{$l->id}}" name="description[]" value="{{$l->description ?? null}}">
            <input  type="hidden" class="link-{{$l->id}}" name="link[]" value="{{$l->link ?? null}}">
            <input  type="hidden" class="icon-{{$l->id}}" name="icon[]" value="{{$l->icon ?? null}}">
              <div style="cursor:move" class="dd-handle dd3-handle"></div><div class="dd3-content">{{$l->name}} <i class="fa fa-angle-right" aria-hidden></i> <code><i>{{$l->link}}</i></code><span style="float:right"><a href="javascript:void(0)" onclick="$('.description').val('{{$l->description}}');$('.link').val('{{$l->link}}');$('.name').val('{{$l->name}}');$('.iconx').val('{{$l->icon}}');$('#type').val('{{$l->id}}');$('.modal').modal('show')" class="text-warning" > <i class="fa fa-edit" aria-hidden></i> </a> &nbsp; <a href="javascript:void(0)" onclick="del_menu('{{$l->id}}')" class="text-danger" > <i class="fa fa-trash" aria-hidden></i> </a></span></div>
          {!!ceksubmenu($menu,$l->id)!!}
