    ...
    @foreach($type as $row)
          <div title="Klik untuk selengkapnya" class="pointer col-md-6 col-lg-4 " onclick="location.href='{{admin_url($row['name'])}}'">
            <div class="widget-small primary coloured-icon"><i class="icon fa {{$row['icon']}} fa-3x"></i>
              <div class="info">
                <h4>{{$row['title']}}</h4>
                <p><b>{{count(collect($posts->where('post_type',$row['name'])))}}</b></p>
              </div>
            </div>
          </div>
          @endforeach
...
<div class="col-lg-8 mb-3">
  ...
  <tbody>
  @foreach(App\Models\Post::with('author')->latest('created_at')->wherein('post_type',$type)->wherePostStatus('publish')->limit(5)->get() as $r )
   <tr>
    <td>{{Str::title($r->post_type)}}</td>
    <td><code>{{time_ago($r->created_at)}}</code></td>
    <td><a href="{{url(admin_path().'/'.$r->post_type.'/edit/'.enc64($r->post_id))}}">{{$r->post_title}}</a>  <i class="text-muted">oleh {{$r->author->name}}</i></td>
   </tr>
  @endforeach
  </tbody>
  </table>
