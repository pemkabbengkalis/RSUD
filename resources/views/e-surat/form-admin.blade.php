...
@foreach(collect(_loop(\App\Models\Posts::wherePostId($edit->post_parent)->select('data_loop')->first()))->where('jenis','!=','Array') as $r)
@if($r->jenis=='Break')
<tr>
  <td colspan="2" align="center" class="bg-warning-light"><b>{{$r->kolom}}</b></td>
</tr>
@else
    <tr>
        <td>{{$r->kolom}}</td>
...
@php 
$fc = collect(_loop(\App\Models\Posts::wherePostId($edit->post_parent)->select('data_loop')->first()))->where('jenis','Array')->first();
@endphp
@if($fc)
<h6>{{$fc->kolom}}</h6>
